...
namespace AutoscaleDemo
{
    ...
    public sealed class Program
    {
        ...
        private float delayBetweenInsertOperations = 0;
        private float[] delayValuesInMs = new float[] { 1000, 125, 125 / 2, 125 / 4, 125 / 8, 0 }; // Different delays between operations - experimentally tuned

        //private int timeBetweenTrafficPatternChangeInSeconds_Demo = 15; // Change traffic pattern every 15 seconds during live demo
        private int timeBetweenTrafficPatternChangeInMinutes_DataGenerator = 8; // Change traffic pattern every 8 minutes to generate history of RU/s.
        ...
        private async Task SetupCosmosResources()
        {
        ...
        private async Task LogOutputStats() // Compute average RU/s consumed across all tasks every 1 second
        {
            ...
            while (this.pendingTaskCount > 0)
            {
                ...
                Console.WriteLine("Inserted {0} docs @ {1} writes/s with average throughput {2} RU/s",
